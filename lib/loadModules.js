...
module.exports = function loadModule(modpath, opt, parent) {
	...
	mod.app = app;
	mod.parent = parent;
	mod.name = isUndef(conf.name) ? p.basename(mod.dir): conf.name;
	mod.prefix = conf.prefix || '';
	mod.isRoot = !parent;
	...
	if (conf.locals) deepMerge(mod.locals, conf.locals);
	merge(app.locals, mod.locals);

	//loadControllers
	mod.controllers = loadControllers(mod, opt);


	var submoddir = p.join(mod.dir, 'modules');
	//load submodule
	if (fs.existsSync(submoddir)) {
		fs.readdirSync(submoddir).forEach(function (subname) {

			var sm = loadModule(p.join(submoddir, subname), opt, mod),
				smpath = getParentsName(mod, sm.prefix + sm.name).join('/');

			//redirect
			if (config.strict_routing) app.all(smpath, strictSubmoduleMiddleware(mod, sm.name));
